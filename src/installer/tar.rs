...
impl TarInstaller {
    pub fn install<P: AsRef<Path>>(path: P, log_tx: Option<std::sync::mpsc::Sender<String>>) -> InstallResult {
        ...
        log::info!("Installing tar.gz bundle: {} -> {}", path.display(), target_dir.display());

        if let Some(ref tx) = log_tx {
            tx.send(format!("Starting installation of tar.gz bundle: {}", app_name)).ok();
        ...
        if let Some(ref tx) = log_tx {
            tx.send(format!("Extracting files using tar...")).ok();
        }
        ...
        let result = SystemExec::run_streaming("tar", [
            "-xzf",
        ...
        match result {
            ...
            Err(e) => {
                fs::remove_dir_all(&temp_dir).ok();
                let err_msg = format!("Failed to run tar: {}", e);
                if let Some(ref tx) = log_tx {
                    tx.send(err_msg.clone()).ok();
                }
                InstallResult {
    ...
    pub fn uninstall(app: &AppEntry) -> InstallResult {
        log::info!("Uninstalling tar.gz app: {}", app.name);
